...
const K = "modulepreload"
    ...
    if (r && r.length > 0) {
        ...
        s = Promise.allSettled(r.map(l => {
            ...
            const f = l.endsWith(".css")
              , g = f ? '[rel="stylesheet"]' : "";
            if (document.querySelector(`link[href="${l}"]${g}`))
                return;
            const u = document.createElement("link");
            if (u.rel = f ? "stylesheet" : K,
            f || (u.as = "script"),
            u.crossOrigin = "",
            u.href = l,
            ...
            f)
                return new Promise( (p, b) => {
                    ...
                    u.addEventListener("error", () => b(new Error(`Unable to preload CSS for ${l}`)))
                }
                )
        }
        ))
  ...
  , re = h("btn-start");
function se() {
    return t.audioCtx || (t.audioCtx = new (window.AudioContext || window.webkitAudioContext)),
    t.audioCtx
}
function m(e, n, r="sine", o=.06) {
    try {
        ...
        i.gain.exponentialRampToValueAtTime(.001, s.currentTime + n),
        a.connect(i),
...
async function ue() {
    const {FilesetResolver: e, HandLandmarker: n} = await X(async () => {
        const {FilesetResolver: o, HandLandmarker: s} = await import("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.18/vision_bundle.mjs");
        return {
            FilesetResolver: o,
            HandLandmarker: s
        }
...
function fe(e) {
    ...
    const i = n[12]
      ...
      , L = l.y < f.y
      , x = g.y < u.y
      , J = Math.abs(r.x - o.x) > .03 || r.y < o.y;
    return Math.hypot(r.x - s.x, r.y - s.y) < .06 && !C && !L && !x ? "pinch" : p && C && L && x && J ? "open_palm" : p && b && M && j ? "index_finger" : !p && !C && !L && !x ? "fist" : "idle"
}
function ge(e) {
    if (e === t.currentGesture)
        return t.previousGesture = e,
        t.gestureStableFrames = 0,
        t.currentGesture;
    e === t.previousGesture ? t.gestureStableFrames++ : (t.previousGesture = e,
    t.gestureStableFrames = 1);
    const n = e === "pinch" ? 3 : 4;
    if (t.gestureStableFrames >= n) {
        const r = t.currentGesture;
        return t.currentGesture = e,
        t.gestureStableFrames = 0,
...
function B(e, n) {
    n === "index_finger" ? ie() : n === "open_palm" ? V() : n === "pinch" ? ce() : e === "index_finger" && ae(),
    e === "index_finger" && t.currentStroke && (t.currentStroke.points.length > 1 && t.strokes.push({
        ...t.currentStroke
    }),
...
function me(e) {
    return t.smoothPos.x += (e.x - t.smoothPos.x) * t.smoothFactor,
...
function pe(e) {
    ...
    t.currentStroke ? t.currentStroke.points.push({
        ...o
    }) : (t.currentStroke = {
        points: [o],
        color: t.activeColor,
...
function ye(e) {
    ...
    for (let c = 0; c < t.strokes.length; c++) {
        ...
        for (const u of l.points) {
            const p = u.x - o.x
              , b = u.y - o.y;
            Math.sqrt(p * p + b * b) >= s ? g.push(u) : (a = !0,
            g.length >= 2 && f.push(g),
            g = [])
        }
        if (g.length >= 2 && f.push(g),
        !(f.length === 0 && l.points.length > 0))
            if (f.length === 1 && f[0].length === l.points.length)
                i.push(l);
            else
                for (const u of f)
                    i.push({
...
function we(e) {
    ...
    d.fill(),
    t.nearestStrokeIdx >= 0 && t.nearestStrokeIdx < t.strokes.length && ve(t.strokes[t.nearestStrokeIdx]),
    y()
}
function be() {
...
function ve(e) {
    if (!(!e || e.points.length < 2)) {
        d.save(),
        d.beginPath(),
        d.moveTo(e.points[0].x, e.points[0].y);
...
function y() {
    ...
    for (const e of t.strokes)
        F(I, e);
    t.currentStroke && t.currentStroke.points.length > 1 && F(I, t.currentStroke, !0)
}
function Pe(e, n, r) {
    for (let o = 0; o < 2; o++)
        t.particles.push({
...
const Le = [[0, 1], [1, 2], [2, 3], [3, 4], [0, 5], [5, 6], [6, 7], [7, 8], [0, 9], [9, 10], [10, 11], [11, 12], [0, 13], [13, 14], [14, 15], [15, 16], [0, 17], [17, 18], [18, 19], [19, 20], [5, 9], [9, 13], [13, 17]];
function xe(e, n) {
    if (!n)
        return;
    e.save(),
...
function E() {
    ...
    const e = v
      , n = performance.now();
    if (w.clearRect(0, 0, t.width, t.height),
    t.showCamera && (w.save(),
    ...
    e.readyState >= 2 && e.currentTime !== R) {
        ...
        if (r.landmarks && r.landmarks.length > 0) {
            const o = r.landmarks[0]
              , s = fe(o)
              , a = ge(s);
            t.isModalOpen || (a === "index_finger" && pe(o),
            a === "open_palm" && ye(o),
            a === "pinch" && we(o),
            a !== "index_finger" && t.currentStroke && t.currentStroke.points.length > 1 && (t.strokes.push({
                ...t.currentStroke
            }),
            t.currentStroke = null)),
            xe(d, o),
            ...
            t.currentGesture !== "none" && (B(t.currentGesture, "none"),
            t.currentGesture = "none"),
            t.currentStroke && t.currentStroke.points.length > 1 && (t.strokes.push({
                ...t.currentStroke
            }),
            t.currentStroke = null,
            y())
...
function Te(e) {
    q.style.setProperty("--swatch-color", e),
    q.dataset.color = e
}
document.querySelectorAll("#color-palette .color-swatch").forEach(e => {
    e.addEventListener("click", () => {
        document.querySelectorAll("#color-palette .color-swatch").forEach(n => n.classList.remove("active")),
...
document.addEventListener("pointerdown", e => {
    !P.contains(e.target) && !z.contains(e.target) && P.classList.remove("mobile-open")
}
);
G.addEventListener("input", () => {
    t.thickness = parseInt(G.value),
    ne.textContent = `${t.thickness}px`
}
);
...
h("btn-undo").addEventListener("click", () => {
    t.strokes.length > 0 && (t.strokes.pop(),
    y(),
    m(500, .08, "sine", .03))
}
);
h("btn-clear").addEventListener("click", () => {
    ...
    t.particles = [],
    y(),
    m(300, .15, "triangle", .04)
}
);
h("btn-camera-toggle").addEventListener("click", () => {
    t.showCamera && t.cameraOpacity > .2 ? (t.cameraOpacity = .15,
    T.textContent = "Camera DIM",
    S.classList.remove("dark-mode")) : t.showCamera && t.cameraOpacity <= .2 ? (t.showCamera = !1,
    t.cameraOpacity = 0,
    T.textContent = "Dark Canvas",
    S.classList.add("dark-mode"),
    h("btn-camera-toggle").classList.remove("active")) : (t.showCamera = !0,
    t.cameraOpacity = .35,
